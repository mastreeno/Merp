    ...
    export default {
        ...
        methods: {
            ...
            loadSettings() {
                ...
                ]).then(([vats, documentTypes, providenceFunds, withholdingTaxes, defaults]) => {
                    self.vatList = vats || []
            ...
            setInvoiceDefaults() {
                if (this.defaults) {
                    if (this.invoice.lineItems.length > 0) {
                        ...
                        this.invoice.lineItems.forEach((l) => {
                    ...
                    if (this.defaults.providenceFund) {
                        this.setInvoiceProvidenceFund(this.defaults.providenceFund)
                    }

                    if (this.defaults.withholdingTax) {
                        this.setInvoiceWithholdingTax(this.defaults.withholdingTax)
                    }
                }
            },
            setInvoiceProvidenceFund(providenceFund) {
                ...
                if (this.invoice.lineItems.length > 0) {
                    ...
                    let providenceFundAmount = totalTaxableAmount * (providenceFund.rate / 100.00)
                    this.invoice.providenceFund.amount = providenceFundAmount.toFixed(2)
                }
            },
            ...
            addNewInvoiceItem() {
                let vat = 0.00
                if (this.defaults) {
                    vat = this.vatList.filter(v => v.id === this.defaults.vatRate.id)[0].rate || 0.00
                }
