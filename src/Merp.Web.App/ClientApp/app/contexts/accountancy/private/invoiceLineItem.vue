<template>
    <div class="form-row form-group">
        ...
        <div class="col-md-2">
            <select class="form-control" v-model="lineItem.vat" v-on:change="fixVat" v-b-tooltip.hover v-bind:title="lineItem.vat.description">
                <option v-for="vat in vatList" v-bind:key="vat.id" v-bind:value="vat">{{ vat.description }}</option>
            </select>
        </div>
        <div class="col-md-1">
...
<script>
    export default {
        ...
        methods: {
            ...
            fixVat() {
                if (this.lineItem.vat) {
                    if (isNaN(this.lineItem.vat.rate)) {
                        this.lineItem.vat.rate = 0
                    }

                    let vat = parseFloat(this.lineItem.vat.rate)
                    this.lineItem.vat.rate = parseFloat(vat.toFixed(2))
                }
