...
namespace Merp.Registry.CommandStack.Sagas
{
        ...
        IHandleMessages<ChangePersonContactInfoCommand>
    {
        ...
        //metodo che riceve il comando e lo esegue
        ...
        public Task Handle(ChangePersonContactInfoCommand message)
        {
            return Task.Factory.StartNew(() => {
                var person = _repository.GetById<Person>(message.PersonId);
                if (person.ContactInfo.PhoneNumber != message.PhoneNumber || person.ContactInfo.MobileNumber != message.MobileNumber || person.ContactInfo.FaxNumber != message.FaxNumber || person.ContactInfo.WebsiteAddress != message.WebsiteAddress || person.ContactInfo.EmailAddress != message.EmailAddress || person.ContactInfo.InstantMessaging != message.InstantMessaging || person.ContactInfo.Linkedin != message.Linkedin)
                {
                    person.SetContactInfo(message.PhoneNumber, message.MobileNumber, message.FaxNumber, message.WebsiteAddress, message.EmailAddress, message.InstantMessaging, message.Linkedin);
                    _repository.Save(person);
                }
            });
        }
